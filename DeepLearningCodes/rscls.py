...
class rscls:
    ...
    def all_sample_row_multi(self,sub=0,sub2=10):
        imx,imy = self.gt.shape
        fp = []
        for j in range(imy//10):
            xy = np.array([sub,j*10])
            fp.append(self.get_patch_even(xy))
        return np.array(fp)
