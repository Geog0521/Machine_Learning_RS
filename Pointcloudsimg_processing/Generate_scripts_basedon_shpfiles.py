...
def generate_clipping_scripts(shps_dire,beclipped_tif,destinat_dire):
    #generate a folder
    ...
    if os.path.exists(shps_dire + "\\" + "arcpro_scripts"):
        pass
    else:
        os.makedirs(shps_dire + "\\" + "arcpro_scripts")

    txt_file = shps_dire + "\\" + "arcpro_scripts" + "\\"+"arcpro_clipping_script.txt"
        ...
        for shp in shps:
            if shp[-4:] == ".shp":
                if True:  # and desc_tif in list_tif:  # "int.tif"
                    ...
                    destination_tif = beclipped_tif
                    clipped_tif = destinat_dire +"\\"+ shp[:-4] + ".tif"
                    # get_mask(tif_path, shpfile)
                    shpfile = shps_dire + "\\" + shp
                    desc_str = "desc = arcpy.Describe(" + "'" + shpfile + "'" + ")" + "\n"
                    f.writelines(desc_str)
...
if __name__ == "__main__":
    ...
    shps_dire = "E:\\Point_clouds_processing_yan\\Areas_Pointclouds\\Area3_Terencegdb\\Area3_C6063_OI_RGB_clip\\Tifimages\\Tifimages_0\\randomclipped_OI_clip_0"

    tobe_clipped_tif = "E:\\Point_clouds_processing_yan\\Areas_Pointclouds\\Area3_Terencegdb\\Area3_C6063_OIC_RGB_clip\\Tifimages\\Tifimages_0\\Area3_C6063_OIC_RGB_clip.tif"
    destination_dire = "E:\\Point_clouds_processing_yan\\Areas_Pointclouds\\Area3_Terencegdb\\Area3_C6063_OIC_RGB_clip\\Tifimages\\Tifimages_0\\randomclipped_OIC_clip_0"

    generate_clipping_scripts(shps_dire, tobe_clipped_tif,destination_dire)

    #we should draw the first line in vertical direction.

    #for generate a batch processing script for clipping a destination .tif file
    # des_directory = "H:/Information_thermodynamics/Sentinel2A/S2_Jiangsu_2" #the directory for all .tif files
    # txt_file = "H:/Information_thermodynamics/Sentinel2A/get_training_samples.txt" # an acrpy script for clipping an image with a .shp file
    # destination_tif = "H:/Information_thermodynamics/Sentinel2A/S2_Jiangsu2_tif/fromglc10v01_32_120.tif"# the destination .tif file
    # batch_processing(des_directory,txt_file,destination_tif)
    # get_mask(img_path,shp_path)


    # father_path = "E:\\Point_clouds_processing_yan\\Areas_Pointclouds\\Area2_Terencegdb"
    ...
    # des_directory= "H:/Information_thermodynamics/Sentinel2A/S2_Jiangsu_2"
    # extract_shpfiles(des_directory, threshold_tif_number=294)

    #get the corresponding shpfiles
    # input_clipped_tif = "H:/Information_thermodynamics/Sentinel2A/S2_Jiangsu_2/267_Training/267clipped.tif"
    # shpfile_path = "H:/Information_thermodynamics/Sentinel2A/S2_Jiangsu_2/267_clipped_shpfile"
    # if os.path.exists(shpfile_path):
    #     pass
    # else:
    #     os.makedirs(shpfile_path)
    # extract_to_shpfile(input_clipped_tif, shpfile_path)

    # shpfile_path = "H:/Information_thermodynamics/Sentinel2A/S2_Jiangsu_2/267_clipped_shpfile/Water.shp"
    # delete_noval_polygons(shpfile_path)
    #generating .shp files